...
### E-108: 런처가 멀쩡히 뜨는 백엔드를 2초 차이로 포기 (2026-08-19)

**증상**: 사용자 지적 — "./start.sh 명령어를 하니까 오류가 뜨네."
`[오류] 백엔드가 준비되지 않았습니다` + 로그 30줄을 뱉고 `exit 1`.

**원인**: **백엔드는 아무 문제가 없었다.** 11:25:40에 떠서 11:28:42에 정상 기동을 마쳤고
그 뒤로도 계속 살아 있었다. 런처의 대기 한도가 `for _ in $(seq 1 180)` — **180초**였을 뿐이다.
그날은 콜드 캐시(NFS에서 모델 로딩)라 기동에 **178초**가 걸렸고, 파이썬 인터프리터 기동
시간까지 더해져 런처가 몇 초 차이로 먼저 포기했다.

과거 기동 시간을 backend.prev-*.log에서 뽑아 보니 한도에 계속 붙어 있었다 —
89 / 90 / 118 / 138 / 144 / 155 / 157 / 162 / **178초**. 캐시가 따뜻하면 90초, 그날 첫
기동이면 178초. **180초 벽은 언제 넘어도 이상하지 않았고, 이날 넘었다.**

**왜 "접속 불가"로 보였나** — 이게 이 사고의 실질적 피해다.
`exit 1`이 6단계(워치독)·7단계(터널) **앞에서** 났다. 그래서 백엔드만 홀로 살아 있고,
cloudflared가 안 떠서 **외부 접속 주소가 아예 없었고**, 워치독도 안 떠서 자동 복구도 없었다.
워치독 로그의 마지막 기록이 8월 12일이었던 이유가 이것이다.

**수정** (`start.sh`):
- 대기 한도 180 → 600초.
- **포기 판단을 시계가 아니라 프로세스 생사로 바꿨다.** 매 초 `pgrep -f 'python.*-m app\.main'`으로
  살아 있는지 보고, 죽었으면 즉시 포기한다(처음 10초는 setsid가 fork 전일 수 있어 유예).
- 검사 `tests/test_start_wait.sh` — 실제 루프를 start.sh에서 떼어내 돌린다.
  20초 뒤 뜨는 백엔드를 놓치지 않는가 / 죽은 백엔드를 600초 채우지 않고 포기하는가(실측 11초).

**교훈**:
- **시간으로 "느린 기동"과 "죽은 기동"을 구분하려 하면 둘 중 하나는 반드시 틀린다.**
  타임아웃을 늘리면 진짜 죽었을 때 오래 매달리고, 줄이면 이 사고가 난다. 판단 근거를
  시계가 아닌 관측 가능한 사실(프로세스 생사)로 바꿔야 한다.
- **한도는 실측 분포 위에서 정할 것.** 최악 178초 위에 180초를 둔 것은 여유가 아니라 우연이었다.
- **실패 경로가 뒤 단계를 통째로 건너뛴다는 점을 볼 것.** 여기서는 "백엔드 확인 실패"가
  터널·워치독까지 날려, 증상이 원인과 전혀 다른 모습(접속 주소 없음)으로 나타났다.
- `pkill -f`/`pgrep -f`는 자기 명령줄까지 매칭한다. 이 사고를 고치는 도중 실제로
  `pkill -f 'python3 -m app.maintest'`가 자기 셸을 죽였다. 패턴에 대괄호를 넣으면
  (`app[.]maintest`) 정규식은 대상에 걸리고 자기 명령줄에는 안 걸린다.

---

### E-109: 딥 리서치가 고른 근거 40건 중 14건만 모델에 도달 (2026-08-19)

**증상**: 사용자 요청 — "딥 리서치 검색 범위를 더 넓게 하고싶은데."
조사해 보니 방 `rp_a3d6845962d442a5`는 이미 `12 / 15 / 3 / 40`으로 **모든 다이얼이 최대치**였다.
더 올릴 곳이 없었다.

**원인**: 다이얼이 아니라 **코드에 박힌 문자 상한**이었다.
`service.py` `_MAX_EVIDENCE_CHARS = 14_000`이 근거 블록을 잘라낸다. 벡터 스토어 실측
청크 중앙값이 **977자**(4천 청크 표본)라 한 항목이 헤더 포함 약 1,017자 —
즉 **14,000자 = 실제로 프롬프트에 들어가는 근거 약 14건**이었다.

그런데 `_rank_sources`가 고른 40건은 **전부** 참고문헌 목록으로 나가고 `turns.sources_json`에도
저장됐다. **모델이 읽지도 않은 26건에 인용 번호가 붙었다.**

추측이 아니다. 납품된 보고서에 모델이 직접 써놨다:
> ※ 최종 점검 의견: 본 분석은 제공된 **[1]~[10]번 문서만**을 대상으로 하였으며,
> **[11]~[24]번 문서는 원문이 제공되지 않아** 분석에서 제외되었습니다.

같이 발견된 두 번째 절단: `_SYNTHESIS_MAX_TOKENS = 4096`에서 **보고서 자체가 잘리고 있었다.**
최근 4건 중 2건이 문장·표 셀 중간에서 끊겼다(`| ② | 저장·전처리·안전성 확보 | 동애`).
6,494자 / 4096토큰 = 1.586자·토큰으로 한글+마크다운 비율과 정확히 맞는다.

컨텍스트는 병목이 **아니었다.** Ollama가 gemma4:31b를 262,144토큰으로 올리고 있고
(`/api/show`, `data/logs/ollama.log`의 `n_ctx = 262144`) 40건은 ~25k토큰으로 10%도 안 썼다.

**수정**:
- `_MAX_EVIDENCE_CHARS` 14,000 → 90,000. `store.LIMITS`의 근거 상한(80)에서 거꾸로 잡았다
  — 80건 × 1,017자 = 81,360자. 두 상수는 한 쌍이라 주석에 그 관계를 남겼다.
- `store.LIMITS["max_evidence_chunks"]` (5, 40) → (5, 80), UI 슬라이더 최대값도 80.
- `_evidence_block`이 **실제로 담은 근거를 함께 반환**하고 호출부가 `sources`를 그것으로
  갈아끼운다. 진행 메시지·done 페이로드·저장 출처·완료 로그가 전부 같은 수를 말한다.
- `_SYNTHESIS_MAX_TOKENS` 4096 → 8192.
- 완료 로그에 근거 문자수 추가 — `근거=%d청크(%d자)`.
- 회귀 테스트 2건: 프롬프트 본문에 `[80]`이 실제로 있는가 / 상한에 걸려 잘린 근거가
  출처 목록에서도 빠지는가.

**교훈**:
- **사용자에게 보여 주는 숫자가 실제로 쓰인 숫자여야 한다.** UI는 "40건까지 씁니다",
  DB에는 40건, 모델이 본 건 14건이었다. 셋이 어긋나는데 아무 데서도 안 보였다.
- **다이얼이 최대치인데 부족하다는 요청은 다이얼 문제가 아니다.** 상한 뒤에 숨은 상수를 찾을 것.
- **파이프라인 끝에서 조용히 자르는 코드는 자른 사실을 반환해야 한다.** `break` 후 그냥
  문자열만 돌려주면 호출자는 무엇이 빠졌는지 영원히 모른다. E-96(노트 자기인용)이 막은 것과
  같은 환각 표면을 반대 방향에서 열어 둔 셈이었다.
- **예산은 로그에 남길 것.** 이번 근본 원인은 어떤 로그에도 어떤 화면에도 안 보이는
  숫자였다. 포맷 지정자 하나면 영구히 관측 가능해진다.
- 사고 5·E-97~100과 같은 성격이다 — **경계에서 끝을 확인하지 않은 결함.**

---

### E-110: 참조 문서 목록이 "모델이 쓴 것"이 아니라 "검색된 것 전부"였다 (2026-08-19)

**증상**: 사용자 지적 — "답변 받을때 분명 참조한 문서가 있는데 본문엔 반영을 안한 듯하게
나오거든." 대화·딥 리서치 양쪽 모두.

**원인**: 두 경로가 각자 다른 이유로 **쓰지도 않은 자료를 출처로 표시**하고 있었다.

*대화* — `upstream_adapter._resolve_inline_markers`가 본문 인용을 검증한 뒤, **인용되지
않은 검색 결과를 답변 끝에 통째로 보탰다.** 근거는 "검색된 자료는 전부 LLM 컨텍스트에
들어갔으므로 실제로 참고된 것"이었는데 그게 사실이 아니다. **컨텍스트에 넣는 것과 모델이
답변에 쓰는 것은 다르다.** `rag_top_k=10`이라 10건이 붙는데 모델은 한둘만 쓴다.

*딥 리서치* — `_rank_sources`가 고른 근거 전부(최대 80건)를 `sources`로 내보냈다.
실측: 근거 40건을 준 보고서가 본문에서 인용한 것은 **22건**, 나머지 18건은 목록에만 있었다.

**곁가지로 드러난 진짜 버그**: 대화 쪽 폴백을 없애자 `test_bracketed_filename_marker_is_matched`가
실패했다. 마커 정규식 `\[\[[^\[\]]*\]{0,2}`는 `[[doc:[이암허브]농식품…]]`에서 `[[doc:`까지만
매치해 마커를 통째로 지운다. **E-106에서 고쳤다고 기록돼 있었지만 인라인에서는 한 번도
작동한 적이 없었고**, 끝에 보태는 폴백이 결과적으로 가려 주고 있었다. 테스트는 폴백 덕분에
통과하고 있었다.

**수정**:
- 대화: 끝에 보태는 블록 삭제. 본문에서 인용하고 검증까지 통과한 것만 남긴다.
  그 블록에서만 쓰이던 `_marker_id`도 함께 제거.
- 대화: 마커 정규식을 `\[\[(?:(?!\]\])[\s\S])*\]{0,2}`로 — `]]`가 나올 때까지 먹으므로
  파일명 안의 `]` 하나에 끊기지 않는다.
- 딥 리서치: 보고서 본문의 `[n]`을 파싱해 인용된 것만 `sources`로 내보낸다.
  **번호는 보고서에 적힌 그대로 유지한다** — 다시 매기면 본문의 `[n]`과 목록이 어긋난다.
- 딥 리서치: 인용이 0건이면 경고 로그. 근거를 줬는데 하나도 안 쓴 보고서라는 뜻이다.
- 인용 정규식은 `[6, 18]` 같은 **묶음 인용**을 처리한다. 단순 `\[(\d+)\]`로 뽑으면
  실측 22건 중 5건이 누락됐다.
- 회귀 테스트: 인용된 것만 반환 / 묶음·환각 번호 처리 / 미인용 자료 미표시.

**교훈**:
- **"컨텍스트에 넣었다"는 "답변에 썼다"가 아니다.** 출처 목록의 의미를 전자로 두면
  사용자에게는 거짓말이 된다. E-109(근거가 잘렸는데 목록엔 남음)와 같은 병이 다른 자리에서
  또 나온 것이다.
- **폴백이 버그를 가린다.** "실패하면 전부 보여 주기"는 안전해 보이지만, 그 아래 정상 경로가
  죽어 있어도 아무도 모른다. 실제로 E-106 수정이 그렇게 2주 넘게 죽어 있었다.
  테스트가 폴백 경로로 통과하고 있지는 않은지 확인할 것.
- **사용자 요청이 바뀔 수 있다.** 끝에 보태는 동작은 원래 사용자 요청("관련된 근거를
  참고했으면 다 달아야지")이었다. 요청의 **의도**(자료 접근 경로 확보)와 **부작용**(안 쓴
  자료가 근거로 보임)을 갈라 봐야 한다.
