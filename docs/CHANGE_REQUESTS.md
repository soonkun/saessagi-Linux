...
## CR-73: 딥 리서치 방 — 입력창 하단 고정 · 목록 최신순 스크롤 (2026-08-12)

**상태**: 완료

**요청**: "여기에 프롬프트 창을 아래로 고정시켜주고 리스트는 최근 것이 젤 위로 가면서
창을 넘어서면 스크롤로 볼 수 있게 하고, 프롬프트 창은 [새싹이] 대화창과 같은 형식을
갖추도록 해줘."

### 원인 — 칼럼에 높이가 없었다

부모(`ChatPanel`·`DesktopView`)는 `flex:1 · overflow:hidden`인 flex 컬럼인데,
방 화면의 `dr-column`에 `height:100%`가 없어 **내용 높이로만 자랐다.** 그래서 목록
바로 뒤에 입력창이 붙고 그 아래가 빈 공간으로 남았다. 목록 영역에 `flex:1`을 줘도
채울 높이 자체가 없으니 아무 효과가 없었다 — flex 자식의 `flex:1`은 **부모의 높이가
확정되어야** 의미가 생긴다.

### 함께 고친 것 — 자동 스크롤 방향

목록은 이미 최신순(`runs`가 `reverse()`로 끝난다)이었는데 자동 스크롤은 **맨 아래로**
내려갔다. 방금 끝난 리서치가 화면 밖으로 밀려나는 셈이다. `top: 0`으로 바꾸고,
의존성을 `steps.length` → `openRun`으로 옮겼다 — 진행 줄이 늘 때마다 목록을 흔들 이유가
없고, 진행 상자는 자기 안에서 따라간다(`stepsBoxRef`).

### 새싹이 대화창과 맞춘 값

- `borderRadius: 26` — 한 줄 높이 52의 절반. `999`로 두면 여러 줄에서 양끝이 부푼다
- `background: var(--color-panel)` + `boxShadow: 0 1px 3px rgba(0,0,0,0.06)`
- `maxWidth: 860` 가운데 정렬, 첨부 버튼 원형 34px, 첨부 칩도 알약 폭에 맞춰 왼쪽 정렬

### 검증 (헤드리스 브라우저, 실측)

| | 결과 |
|---|---|
| 입력창 하단 고정 | 화면 900 → 하단 y **888** · 화면 460 → **448** |
| 목록 최신순 | 맨 위 `8.10 13:23` · 맨 아래 `8.9 09:00` |
| 창 넘어가면 스크롤 | 화면 460에서 `overflowY:auto` 컨테이너 스크롤 가능 |
| 알약 모양 | radius 26px · panel 배경 · 그림자 있음 (ChatPanel과 동일) |
| JS 오류 | 없음 |
